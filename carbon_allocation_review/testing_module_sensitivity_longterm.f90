program test_storage_allocation_sensitivity
  ...
  open(newunit=checkpoint_unit, file="storage_allocation_sensitivity_checkpoints.csv", &
       status="replace", action="write")
...
contains
  ...
  subroutine run_scenario(scenario_id, params, controls, initial_state, npp_rate, &
    ...
    do day = 1, n_days
       ...
       summary%cumulative_starvation_carbon_loss = summary%cumulative_starvation_carbon_loss + &
                                                  result%starvation_carbon_loss
       summary%cumulative_sapwood_to_heartwood = summary%cumulative_sapwood_to_heartwood + &
       ...
       if (is_checkpoint_day(day)) then
          call write_checkpoint_row(checkpoint_unit, summary, params, state, &
                                    carbon_storage, day, positive_npp_integral)
       end if
  ...
  subroutine initialize_summary(summary, scenario_id, params, controls, initial_state, &
    ...
    summary%cumulative_starvation_carbon_loss = 0.0_real64
    summary%cumulative_sapwood_to_heartwood = 0.0_real64
    summary%cumulative_unpaid_carbon_deficit = 0.0_real64
  ...
  function step_is_valid(state, result, carbon_storage, failure_reason) result(is_valid)
    ...
    if (result%leaf_starvation_loss < -tol .or. &
        ...
        result%unpaid_carbon_deficit < -tol) then
       is_valid = .false.
  ...
  function is_checkpoint_day(day) result(is_checkpoint)
    ...
    ! Checkpoints are saved at the end of years 1, 5, and 10.
    is_checkpoint = (day == 365) .or. (day == 365 * 5) .or. (day == 365 * 10)
  ...
  subroutine write_checkpoint_header(checkpoint_unit)
    ...
    write(checkpoint_unit,'(a)') &
      ...
      "cumulative_starvation_carbon_loss,cumulative_sapwood_to_heartwood," // &
      "cumulative_unpaid_carbon_deficit," // &
      "leaf_root_residual,pipe_residual,storage_fraction," // &
      ...
      "max_storage_fraction,min_living_pool,days_with_allocation," // &
      "days_with_unmet_storage_deficit"
  ...
  subroutine write_checkpoint_row(checkpoint_unit, summary, params, state, &
    ...
    if (positive_npp_integral > 0.0_real64) then
       structural_fraction_at_checkpoint = summary%cumulative_structural_allocation / &
                                           positive_npp_integral
    else
    ...
    write(checkpoint_unit,'(*(g0))') &
      ...
      summary%cumulative_sapwood_to_heartwood, ",", &
      summary%cumulative_unpaid_carbon_deficit, ",", &
      current_leaf_root_residual, ",", current_pipe_residual, ",", &
      current_storage_fraction, ",", structural_fraction_at_checkpoint, ",", &
      summary%max_abs_structural_balance_error, ",", &
      summary%max_abs_storage_balance_error, ",", &
      summary%max_abs_whole_plant_balance_error, ",", &
      summary%max_storage_fraction, ",", summary%min_living_pool, ",", &
      summary%days_with_allocation, ",", summary%days_with_unmet_storage_deficit
